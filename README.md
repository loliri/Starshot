...
- **Custom Wallpaper**: Three modes
  ...
  - **Random from Folder**: Picks a random image or video from a folder on each launch; an optional "Prefer video" sub-toggle prefers videos when on, falling back to a random image if the folder has no video or the video fails to initialize.
...
- Video wallpaper may fail to initialize on startup due to MF media pipeline contention (intermittent, not fully resolved); auto-rebuilds up to 2 times, then falls back to a random image — no black screen.
