...
<img src="../src/logo.png" width="300" alt="Logo de Starshot">
...
> [!WARNING]
> Este documento ya no se mantiene y su contenido puede estar desactualizado. Se recomienda leer directamente la [documentación en inglés](../README.md), o utilizar IA / software de traducción para leer la versión en inglés.
...
- 🗂️ **Soporte multiformato**: AVIF / JPEG XL / PNGv3 / Ultra HDR JPEG / PNG, incluyendo herramienta de conversión por lotes.
...
| Ultra HDR JPEG  | 8 bits + gain map               | Respaldo HDR compatible con SDR                  | Salida HDR adicional                 |
...
- **Conversión de formato**: modo de pantalla HDR → AVIF / JPEG XL; modo de pantalla SDR + origen HDR → SDR JPEG / Ultra HDR JPEG / SDR PNG (salidas con mapeo tonal WYSIWYG); origen SDR → PNG / AVIF / JPEG XL
- **Gestión del color**: lee el perfil ICC de la pantalla y AdvancedColorInfo

### Conversión por lotes de formato

Formatos de salida: SDR JPG / SDR PNG / AVIF / JPEG XL / Ultra HDR JPG, calidad por defecto 100 (nivel sin pérdidas).

| Dirección de conversión              | Motor                                 |
| ------------------------------------ | ------------------------------------- |
| JPG / PNG → AVIF / JXL               | avifenc.exe / cjxl.exe (CLI)          |
| AVIF / JXL → JPG / PNG               | Decodificación en proceso + HDR por la misma línea de mapeo tonal que las capturas SDR directas |
| JXR / WEBP / HEIC etc. → AVIF / JXL  | ImageSaver en proceso (avifEncoderLite) |
| Cualquiera → Ultra HDR JPG           | ImageSaver en proceso (UhdrEncoder)   |
...
| Códecs                   | Starward.Codec NuGet (wrapper P/Invoke de libavif / libjxl / Ultra HDR)  |
...
|                       | Debug                              | Release                                            |
| --------------------- | ---------------------------------- | -------------------------------------------------- |
| .NET Runtime          | Dependiente del framework       | Autocontenido                                      |
| Librerías nativas     | Solo win-x64                    | Igual que Debug; arm64 requiere `-r win-arm64` explícito |
| Trim                  | No aplicado (sin recorte)          | Parcial                                            |
| Optimizador AOT CsWinRT | Desactivado (compilación más rápida) | Activado — garantiza que la interoperabilidad WinRT sobreviva al trimming |
| ReadyToRun            | No aplicado (JIT estándar)         | Precompilado AOT                                   |
| Ruta de salida        | `build/app/`                       | `build/release/app/`; el lanzador se copia a `build/release/` si se compiló antes |
| Tamaño                | ~80 MB                           | ~160 MB (Trim)                                     |

## Compilar desde el código fuente

### Requisitos previos

- Visual Studio 2026 (con Desarrollo de escritorio C++ y Desarrollo de escritorio .NET)
- .NET 10 SDK
...
Las traducciones se basan en los archivos `.resx` de `src/Starshot.Language/` (`Lang.resx` es el inglés por defecto; `Lang.zh-CN.resx` etc. por idioma). También hay que añadir una opción al ComboBox de idioma en `GeneralSetting` + su mapeo `LanguageIndex`.

Se aceptan contribuciones de traducción: haz fork del repositorio → copia `Lang.resx` a `Lang.{tu-locale}.resx` → traduce → abre un PR.
