...
- 📋 **原生剪貼簿**——Win32 原生 API 直寫剪貼簿，貼上可靠不丟內容
...
- **自訂桌布**：三種模式
  ...
  - **資料夾隨機**：每次啟動從資料夾隨機抽一張（圖片或影片混選）；附帶「優先影片」子開關，開啟時優先抽影片，若資料夾內無影片或影片初始化失敗則回退為隨機圖片
...
- 影片桌布啟動時可能因 MF 媒體管線競爭而初始化失敗（間歇性，未完全解決）；自動重建最多 2 次後回退為隨機圖片——不會黑屏
