...
- 📋 **ネイティブクリップボード** — Win32 ネイティブ API でクリップボードに直接書き込み、確実に貼り付け可能
...
- **カスタム壁紙**：3 つのモード
  ...
  - **フォルダランダム**：起動ごとにフォルダからランダムに 1 枚（画像または動画を混在選択）；オプションの「動画優先」サブスイッチをオンにすると動画を優先的に選択し、フォルダに動画がない、または動画の初期化に失敗した場合はランダムな画像にフォールバック
...
- デュアルモニターで DPI が異なる場合（例: プライマリ 150%、セカンダリ 125%）、領域キャプチャの**ウィンドウ検出**（hover ハイライトでウィンドウを選択）がセカンダリモニターで座標ずれを起こします。ドラッグ自由選択や保存には影響しません。回避策: 両モニターのスケールを統一する
...
- 動画壁紙は起動時に MF メディアパイプラインの競合により初期化に失敗することがあります（断続的、完全には解決されていません）；最大 2 回まで自動再構築したのちランダムな画像にフォールバック——黒画面にはなりません
