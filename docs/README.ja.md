...
> [!WARNING]
> このドキュメントは更新を終了しました。内容が最新版から遅れている可能性があります。[英語版](../README.md)を直接お読みいただくか、AI・翻訳ツールを使って英語版をご覧ください。
...
4. **カラーマネジメント**：ディスプレイ ICC プロファイルを読み取り、実際の色域原色を解析し、ファイルの cICP/ICC チャンクに書き込みます。HDR は常に BT.2020 に固定。SDR はデフォルトでオフ（BT.709）、オプションで有効化可能（ICC プロファイルから実際の色域を読み取り）——有効化前にモニターのカラー設定を検証し、無効な場合（VM や ICC プロファイルのないデバイスなど）は有効にできません
...
- **マルチディスプレイ**：仮想画面全体をカバー。選択範囲は画面をまたいで指定可能（HDR+SDR 混合でも明るさ正確）。拡大鏡と座標ボックスはカーソルのあるディスプレイのみ
...
- Windowsクリップボード履歴（Win+V）の画像アイテムを独立ページで閲覧
- `Clipboard.GetHistoryItemsAsync`で読み込み、時系列で表示
- クリップボード変更時に自動更新（ContentChanged + スロットル）+ ウィンドウアクティブ時に更新
- クリックでプレビュー（画像ビューア、前/次ナビゲーション対応）
- コンテキストメニュー：情報（フォーマット/サイズ/寸法）/ 開く / 再コピー / 履歴から削除
- Windows設定でクリップボード履歴を有効化する必要があります
- 無効時はプロンプト + `ms-settings:clipboard`リンク、画像なし時は空状態メッセージ

### 画像ビューア

- ズーム（スライダー / ボタン / マウスホイール スムーズアニメーション / ダブルクリックでフィット）、フルスクリーンモード（F11）
...
- **カスタム壁紙**：3 つのモード
  ...
  - **フォルダランダム**：起動ごとにフォルダからランダムに 1 枚（画像または動画を混在選択）；オプションの「動画優先」サブスイッチをオンにすると動画を優先的に選択
...
- 更新は連鎖デルタパッケージを優先（変更ファイルのみ、全体約 80MB に対して数 MB）；現在のバージョンから上向きに階層ごとに照合、最大 5 階層まで（設定 → 詳細で調整可能）；階層が多すぎるか一致が見つからない場合は完全パッケージにフォールバック
- **手動フル更新**：更新ウィンドウのメインボタンのドロップダウンから「フル更新」を選択可能、デルタをスキップして完全パッケージを直接ダウンロード
- 完全・デルタ両パッケージとも SharpCompress の真のストリーミング解凍（ネットワークストリーム直結、zip をディスクに保存しない）を使用し、エントリごとに直接ルートディレクトリに書き込み
...
<details>
<summary><b>スクリーンショットの保存がクラッシュする（VM / 一部のモニター）</b></summary>

これらの環境（VM、ICC プロファイルのないデバイス）では、モニターのカラー設定が無効なものとして報告されます。カラーマネジメントがオンの場合、エンコーダー（lcms2）が不正な色域データを処理する際にクラッシュします。これを回避するにはカラーマネジメントをオフ（デフォルト）のままにしてください。HDR スクリーンショットには影響しません。
