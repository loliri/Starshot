...
- 失敗時は復元、成功時はランチャーを `--cleanup-old` 付きで再起動して古いバージョンをクリーンアップ
...
C++ ネイティブプログラム（~400KB）。`version.ini` を読み取り、`app-{version}/Starshot.exe` を起動するか（version.ini がない場合は `app/`、debug/local ビルド）を決定します。`--cleanup-old` パラメータ付きで起動すると、`app-*` ディレクトリを走査して現在のバージョン以外を削除します。
