...
{
    private static readonly Microsoft.Extensions.Logging.ILogger _logger = LoggerFactory
        .Create(b => b.AddSerilog())
        .CreateLogger("UpdateService");

    public static async Task<(ReleaseInfo? update, string? latestTag)> CheckUpdateAsync(
        bool ignoreSkipped = true
    )
    {
#if DEBUG
        return (null, null);
        ...
        var release =
            AppConfig.UpdateSource == 0
                ? await ReleaseClient.GetLatestReleaseCDNAsync(
                    AppConfig.EnablePreReleaseUpdateCheck
                )
                : await ReleaseClient.GetLatestReleaseGitHubAsync(
                    AppConfig.EnablePreReleaseUpdateCheck
                );
        if (release is null)
            return (null, null);
        if (!TryParseVersion(AppConfig.AppVersion, out var current))
            return (null, release.TagName);
        if (release.Version <= current)
            return (null, release.TagName);
        // 只有自动检查才跳过用户忽略的版本；手动检查无视忽略
        if (
            ignoreSkipped
            && SemVersion.TryParse(AppConfig.IgnoreVersion, out var ignore)
            && release.Version <= ignore
        )
            return (null, release.TagName);
    ...
    public static async Task ExtractToDirectoryAsync(
        string zipUrl,
        string destDir,
        IProgress<(int percent, string bytesText)>? progress,
        CancellationToken ct = default,
        bool disableCert = false
    )
    {
        string destFull =
        ...
        while (reader.MoveToNextEntry())
        {
            ...
            string dest = Path.GetFullPath(
                Path.Combine(destDir, key.Replace('/', Path.DirectorySeparatorChar))
            );
            ...
            while ((n = await entryStream.ReadAsync(buf, 0, buf.Length, ct)) > 0)
            {
                ...
                if (total > 0)
                {
                    ...
                    progress?.Report(
                        (pct, $"{FormatSize(counting.BytesRead)} / {FormatSize(total.Value)}")
                    );
                }
            }
        }
    ...
    )
    {
        ...
        try
        {
            ...
            try
            {
                File.WriteAllText(versionIni, $"version={info.TagName}");
            }
            catch { }

            Process.Start(
                new ProcessStartInfo(launcherExe)
                {
                    UseShellExecute = true,
                    Arguments = $"--clean={Environment.ProcessId}",
                }
            );
        ...
        catch (Exception)
        {
            ...
            try
            {
                File.WriteAllText(versionIni, $"version={AppConfig.AppVersion}");
            }
            catch { }
            throw;
        }
    ...
    )
    {
        ...
        if (File.Exists(versionIni))
        {
            ...
            var eq = line.IndexOf('=');
            if (eq >= 0)
                currentTag = line[(eq + 1)..].Trim().ToLowerInvariant();
        }
        ...
        // 查 delta 链
        int maxLayers = 1;
        var chain = await ReleaseClient.GetDeltaChainGitHubAsync(
            currentTag,
            info.TagName,
            maxLayers,
            ct
        ...
        if (chain is null || chain.Count == 0)
        {
            _logger?.LogInformation(
                "Delta skipped: no chain found from {From} to {To} (max {Max} layers), falling back to full package",
                currentTag,
                info.TagName,
                maxLayers
            );
        ...
        if (!Directory.Exists(currentAppDir))
        {
            ...
            {
                _logger?.LogInformation(
                    "Delta skipped: current app dir not found for tag {Tag}, falling back to full package",
                    currentTag
                );
                return false;
            }
        ...
        _logger?.LogInformation(
            "Delta update: {Chain} layers from {From} to {To}",
            chain.Count,
            currentTag,
            info.TagName
        );
        ...
        for (int i = 0; i < chain.Count; i++)
        {
            ...
            else
            {
                ...
                {
                    int pct =
                        basePercent + (int)((double)p.percent / 100 * (nextPercent - basePercent));
            ...
            string patchFile = Path.Combine(
                root,
                ".diff-" + Guid.NewGuid().ToString("N") + ".patch"
            );
            try
            {
            ...
            if (i > 0)
                try
                {
                    Directory.Delete(currentApp, recursive: true);
                }
                catch { }
            currentApp = nextApp;
        }
    ...
    )
    {
        ...
        {
            _logger?.LogInformation(
                "CDN delta skipped: version manifest not found for {Tag}",
                info.TagName
            );
            return false;
        }
        ...
        // diffs 找 from == 当前版本
        var diff = archManifest.Diffs?.FirstOrDefault(d =>
            string.Equals(d.From, currentTag, StringComparison.OrdinalIgnoreCase)
        );
        if (diff is null)
        {
            _logger?.LogInformation(
                "CDN delta skipped: no diff from {From} to {To} (not in 5 targets)",
                currentTag,
                info.TagName
            );
            return false;
        ...
        if (!Directory.Exists(currentAppDir))
        {
            ...
            {
                _logger?.LogInformation(
                    "CDN delta skipped: current app dir not found for tag {Tag}",
                    currentTag
                );
                return false;
            }
        ...
        if (!string.IsNullOrEmpty(archManifest.Launcher))
        {
            ...
            {
                string launcherUrl =
                    $"{AppConfig.CdnBase}/release/{info.TagName}/{archManifest.Launcher}";
            ...
            catch (OperationCanceledException)
            {
                ...
                try
                {
                    if (File.Exists(tmpLauncher))
                        File.Delete(tmpLauncher);
                }
                catch { }
                throw;
            }
    ...
    private static bool TryParseVersion(string? raw, out SemVersion version)
    {
        ...
        if (s.StartsWith("v", StringComparison.OrdinalIgnoreCase))
            s = s[1..];
        if (SemVersion.TryParse(s, out var v))
            version = v;
        return true;
    }
