...
internal static class ImageThumbnail
{
    ...
    private const string AvifImageBase64 =
        "";

    private const string JxlImageBase64 = "/woAEBAUNwIIBAEAGABLGIsVggE=";

    private static List<KeyValuePair<string, BitmapTypedValue>> ImageQuality =
    [
        new KeyValuePair<string, BitmapTypedValue>(
            "ImageQuality",
            new BitmapTypedValue(0.6f, Windows.Foundation.PropertyType.Single)
        ),
    ];

    private static readonly SemaphoreSlim Semaphore = new(
        Math.Clamp(Environment.ProcessorCount / 2, 1, int.MaxValue)
    );
    ...
    static ImageThumbnail()
    {
        try
        {
            ...
            BitmapDecoder
                .CreateAsync(new MemoryStream(bytes).AsRandomAccessStream())
                .AsTask()
                .Wait();
    ...
    public static async Task<BitmapSource> GetImageThumbnailAsync(
        string path,
        bool png,
    ...
    private static async Task<BitmapSource> LoadAvifImageAsync(
        string filePath,
        string cachePath,
        CancellationToken cancellationToken = default
    )
        ...
        await Task.Run(
            async () =>
            {
                ...
                using var softwareBitmap = new SoftwareBitmap(
                    depth == 8 ? BitmapPixelFormat.Rgba8 : BitmapPixelFormat.Rgba16,
                    (int)rgb.Width,
                    (int)rgb.Height
                );
                using (var buffer = softwareBitmap.LockBuffer(BitmapBufferAccessMode.Write))
                {
                    ...
                    if (capacity < pixelBytes.Length)
                    {
                        throw new InvalidOperationException(
                            "Buffer size is smaller than pixel data size."
                        );
                    }
                    unsafe
                    {
                        NativeMemory.Copy(
                            Unsafe.AsPointer(ref MemoryMarshal.GetReference(pixelBytes)),
                            ptr.ToPointer(),
                            (nuint)pixelBytes.Length
                        );
                    }
                }
                cancellationToken.ThrowIfCancellationRequested();
    ...
    private static async Task<BitmapSource> LoadJxlImageAsync(
        string filePath,
        string cachePath,
        CancellationToken cancellationToken = default
    )
    ...
    private static async Task<BitmapSource> LoadImageAsync(
        string filePath,
        string cachePath,
        CancellationToken cancellationToken = default
    )
    {
        ...
        BitmapDecoder decoder = await BitmapDecoder
            .CreateAsync(fs.AsRandomAccessStream())
            .AsTask(cancellationToken);
    ...
    private static async void SaveCachedImage(
        string path,
    ...
    )
    {
        try
        {
            ...
            BitmapEncoder encoder = png
                ? await BitmapEncoder
                    .CreateAsync(BitmapEncoder.PngEncoderId, fs.AsRandomAccessStream())
                    .AsTask(cancellationToken)
                    .ConfigureAwait(false)
                : await BitmapEncoder
                    .CreateAsync(
    ...
    public static async Task<BitmapSource> GetVideoThumbnailAsync(
        string path,
    ...
    )
    {
        ...
        try
        {
            if (Path.GetExtension(path).Equals(".webm", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    return await GetVideoThumbnailFromMediaPlayerAsync(
                        path,
                        cachePath,
                        cancellation
                    );
    ...
    private static async Task<BitmapSource> GetVideoThumbnailFromMediaPlayerAsync(
        string path,
        string cachePath,
        CancellationToken cancellation = default
    )
    {
        ...
        try
        {
            ...
            player.VideoFrameAvailable += (s, e) =>
            {
                s.Pause();
