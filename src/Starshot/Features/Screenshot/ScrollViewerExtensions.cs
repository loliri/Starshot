...
{
    private static readonly Guid IScrollViewerPrivateGuid = new Guid(
        "7cc9bcb8-947e-582a-ae91-6a76597477c9"
    );

    public static bool GetArePointerWheelEventsIgnored(this ScrollViewer scrollViewer)
    {
        IObjectReference iScrollViewerPrivate = (
            (IWinRTObject)scrollViewer
        ).NativeObject.As<IUnknownVftbl>(IScrollViewerPrivateGuid);
        return get_ArePointerWheelEventsIgnored(iScrollViewerPrivate);
    }

    public static unsafe void SetArePointerWheelEventsIgnored(
    ...
    private static unsafe bool get_ArePointerWheelEventsIgnored(IObjectReference _obj)
    {
        ...
        ExceptionHelpers.ThrowExceptionForHR(
            (
                (delegate* unmanaged[Stdcall]<IntPtr, byte*, int>)(
                    *(IntPtr*)(
                        *(IntPtr*)(void*)thisPtr
                        + 7 * (nint)sizeof(delegate* unmanaged[Stdcall]<IntPtr, byte*, int>)
                    )
                )
            )(thisPtr, &b)
        );
        ...
        ExceptionHelpers.ThrowExceptionForHR(
            (
                (delegate* unmanaged[Stdcall]<IntPtr, byte, int>)(
                    *(IntPtr*)(
                        *(IntPtr*)(void*)thisPtr
                        + 8 * (nint)sizeof(delegate* unmanaged[Stdcall]<IntPtr, byte, int>)
                    )
                )
            )(thisPtr, value ? ((byte)1) : ((byte)0))
        );
    }
}
